...
internal class GetTransactionsByDateQueryHandler : IRequestHandler<GetTransactionsByDateQuery, BaseResponse<IEnumerable<TransactionDto>>>
{
    private readonly ITrasanctionRepository _trasanctionRepository;

    public GetTransactionsByDateQueryHandler(ITrasanctionRepository trasanctionRepository)
    {
        _trasanctionRepository = trasanctionRepository;
    }

    public async Task<BaseResponse<IEnumerable<TransactionDto>>> Handle(GetTransactionsByDateQuery request, CancellationToken cancellationToken)
    {
        var response = new BaseResponse<IEnumerable<TransactionDto>>();
        try
        {
            var transactions = await _trasanctionRepository.GetByDate(request.Date, cancellationToken);
