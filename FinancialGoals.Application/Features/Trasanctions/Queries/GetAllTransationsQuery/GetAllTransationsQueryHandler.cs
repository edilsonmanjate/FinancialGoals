...
public class GetAllTransationsQueryHandler : IRequestHandler<GetAllTransationsQuery, BaseResponse<IEnumerable<TransactionDto>>>
{
    private readonly ITrasanctionRepository _trasanctionRepository;

    public GetAllTransationsQueryHandler(ITrasanctionRepository trasanctionRepository)
    {
        _trasanctionRepository = trasanctionRepository;
    }

    public async Task<BaseResponse<IEnumerable<TransactionDto>>> Handle(GetAllTransationsQuery request, CancellationToken cancellationToken)
    {
        var response = new BaseResponse<IEnumerable<TransactionDto>>();
        try
        {
            var transactions = await _trasanctionRepository.GetAll(cancellationToken);
